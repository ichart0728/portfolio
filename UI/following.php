...
<body style="background-color: #161B21;" id="page-top">
  ...
  <?php
        ...
        foreach($rows as $row){
            ...
            $userid = $row['user_id'];
            $randomPic = $user->getRandomMoviePicture($userid);
        echo
...
<?php
                  $userid = $_SESSION['user_id'];
                  $rows = $user->followingDisplay($userid);
                  if($rows == false){
                    echo "<h4 class='w-5 mx-auto text-center mt-5' style='color:#F5A431;'>No Following</h4>";
                  }else{
                    foreach($rows as $row){
                      $username = $row['username'];
                      $icon = $row['icon'];
                      ...
                      $others = $row['user_id'];
                      // $rows2 = $user->getRandomMoviePicture($others);
                      // $randomPicture = $rows2['picture'];
