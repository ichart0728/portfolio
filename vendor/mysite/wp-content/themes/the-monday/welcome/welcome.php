<?php
	if(!class_exists('Accesspress_Basic_Welcome')) :

		class Accesspress_Basic_Welcome {
			...
			public function __construct() {
				...
				$this->req_plugins = array(
					...
					'instant-demo-importer' => array(
						...
						'name' => __('Instant Demo Importer', 'the-monday'),
						...
						'info' => __('Instant Demo Importer Plugin adds the feature to Import the Demo Conent with a single click.', 'the-monday'),
					),
				...
				$this->tab_sections = array(
					'getting_started' => __('Getting Started', 'the-monday'),
					'recommended_plugins' => __('Recommended Plugins', 'the-monday'),
					'support' => __('Support', 'the-monday'),
					'free_vs_pro' => __('Free Vs Pro', 'the-monday'),
					'more_wp_stuff' => __('More WordPress Stuff', 'the-monday'),
				);
				...
				add_action( 'load-themes.php', array( $this, 'themonday_activation_admin_notice' ) );

				/* Create a Welcome Page */
				add_action( 'admin_menu', array( $this, 'themonday_welcome_register_menu' ) );

				/* Enqueue Styles & Scripts for Welcome Page */
				add_action( 'admin_enqueue_scripts', array( $this, 'themonday_welcome_styles_and_scripts' ) );

				/** Plugin Installation Ajax **/
				add_action( 'wp_ajax_themonday_plugin_installer', array( $this, 'themonday_plugin_installer_callback' ) );

				/** Plugin Installation Ajax **/
				add_action( 'wp_ajax_themonday_plugin_offline_installer', array( $this, 'themonday_plugin_offline_installer_callback' ) );

				/** Plugin Activation Ajax **/
				add_action( 'wp_ajax_themonday_plugin_activation', array( $this, 'themonday_plugin_activation_callback' ) );

				/** Plugin Activation Ajax (Offline) **/
				add_action( 'wp_ajax_themonday_plugin_offline_activation', array( $this, 'themonday_plugin_offline_activation_callback' ) );
			...
			public function get_required_plugin_notification() {
				...
				$themonday_plugin_installed_notif = get_option('themonday_plugin_installed_notif');
				...
				update_option('themonday_plugin_installed_notif', $notif_counter);
				return $notif_counter;
			}

			/** Welcome Message Notification on Theme Activation **/
			public function themonday_activation_admin_notice() {
				global $pagenow;

				if( is_admin() && ('themes.php' == $pagenow) && (isset($_GET['activated'])) ) {
					?>
					<div class="notice notice-success is-dismissible">
						<p><?php /* translators: %1$s : theme name, %2$s : welcome page link */ printf( __( 'Welcome! Thank you for choosing %1$s! Please make sure you visit our <a href="%2$s">Welcome page</a> to get started with %1$s.', 'the-monday' ), esc_html($this->theme_name), esc_url(admin_url( 'themes.php?page=themonday-welcome' ))  ); ?></p>
						<p><a class="button" href="<?php echo esc_url(admin_url( 'themes.php?page=themonday-welcome' )); ?>"><?php esc_html_e( 'Lets Get Started', 'the-monday' ); ?></a></p>
					</div>
					<?php
				}
			}
			...
			public function themonday_welcome_register_menu() {
				$action_count = get_option('themonday_plugin_installed_notif');
				$title        = $action_count > 0 ? esc_html__( 'Welcome', 'the-monday' ) : esc_html__( 'Welcome', 'the-monday' );
				add_theme_page( 'Welcome', $title , 'edit_theme_options', 'themonday-welcome', array( $this, 'themonday_welcome_screen' ));
			}

			/** Welcome Page **/
			public function themonday_welcome_screen() {
				...
				<div class="wrap about-wrap access-wrap">
					<h1><?php /* translators: %1$s : theme name, %2$s : theme version */ printf( esc_html__( 'Welcome to %1$s - Version %2$s', 'the-monday' ), esc_html($this->theme_name), esc_html($this->theme_version) ); ?></h1>
					<div class="about-text"><?php /* translators: %s : theme name */ printf( esc_html__( 'The %s is an elegant responsive WordPress theme for any type of business or freelancers. It is feature-rich, multi-purpose and powerful WordPress theme with a beautiful and user-friendly design. It offers a complete customization capability and multiple options for building a website instantly.', 'the-monday' ), esc_html($this->theme_name) ); ?></div>
				...
				<div class="nav-tab-wrapper clearfix">
					<?php foreach($tabs as $id => $label) : ?>
						...
						?>
						<a href="<?php echo esc_url(admin_url('themes.php?page=themonday-welcome&section='.$id)); ?>" class="<?php echo esc_attr($nav_class); ?>" >
			...
			public function themonday_welcome_styles_and_scripts() {
				...
				wp_localize_script( 'accesspress-basic-welcome-screen', 'themondayWelcomeObject', array(
					'admin_nonce'	=> wp_create_nonce('themonday_plugin_installer_nonce'),
					'activate_nonce'	=> wp_create_nonce('themonday_plugin_activate_nonce'),
					'ajaxurl'		=> esc_url( admin_url( 'admin-ajax.php' ) ),
					'activate_btn' => __('Activate', 'the-monday'),
					'installed_btn' => __('Activated', 'the-monday'),
					'demo_installing' => __('Installing Demo', 'the-monday'),
					'demo_installed' => __('Demo Installed', 'the-monday'),
					'demo_confirm' => __('Are you sure to import demo content ?', 'the-monday'),
				) );
			}

			/** Plugin API **/
			public function themonday_call_plugin_api( $plugin ) {
			...
			/** Check For Icon **/
			public function themonday_check_for_icon( $arr ) {
				if ( ! empty( $arr['svg'] ) ) {
			...
			public function themonday_plugin_active($plugin) {
				$folder_name = $plugin['slug'];
				$file_name = $plugin['filename'];
				$status = 'install';
			...
			public function themonday_plugin_generate_url($status, $plugin) {
				$folder_name = $plugin['slug'];
				$file_name = $plugin['filename'];
			...
			/* ========== Plugin Installation Ajax =========== */
			public function themonday_plugin_installer_callback(){

				if ( ! current_user_can('install_plugins') )
					wp_die( esc_html__( 'Sorry, you are not allowed to install plugins on this site.', 'the-monday' ) );
				...
				if (! wp_verify_nonce( $nonce, 'themonday_plugin_installer_nonce' ))
					wp_die( esc_html__( 'Error - unable to verify nonce, please try again.', 'the-monday') );
				...
				// Get Plugin Info
				$api = $this->themonday_call_plugin_api($plugin);
			...
			public function themonday_plugin_offline_installer_callback() {
			...
			public function themonday_plugin_offline_activation_callback() {
			...
			/** Plugin Activation Ajax **/
			public function themonday_plugin_activation_callback(){

				if ( ! current_user_can('install_plugins') )
					wp_die( esc_html__( 'Sorry, you are not allowed to activate plugins on this site.', 'the-monday' ) );

				$nonce = isset( $_POST["nonce"] ) ? sanitize_text_field( wp_unslash( $_POST["nonce"] ) ) : '';
				$plugin = isset( $_POST["plugin"] ) ? sanitize_text_field( wp_unslash( $_POST["plugin"] ) ) : '';

				// Check our nonce, if they don't match then bounce!
				if (! wp_verify_nonce( $nonce, 'themonday_plugin_activate_nonce' ))
					die( esc_html__( 'Error - unable to verify nonce, please try again.', 'the-monday' ) );
				...
				// Get Plugin Info
				$api = $this->themonday_call_plugin_api(esc_attr($plugin));
				...
				if($api->name){
					...
					$status = 'failed';
					$msg = esc_html__('There was an error activating $api->name', 'the-monday');
				}
			...
			public static function get_plugin_file( $plugin_slug ) {
	      	...
	      	public function get_local_dir_path($plugin) {
	      		...
	      		$url = wp_nonce_url(admin_url('themes.php?page=themonday-welcome&section=import_demo'),'themonday-file-installation');
	...
	if(class_exists('Instant_Demo_Importer')) :
		...
		$demoimporter->demos = array(
			'the-monday' => array(
				'title' => __('Monday Demo', 'the-monday'),
				'name' => 'the-monday',
				'screenshot' => get_template_directory_uri().'/welcome/demos/the-monday/screen.png',
				'home_page' => 'home',
				'menus' => array(
